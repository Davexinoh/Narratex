...
def ask_gemini(question, narratives, system=None):
    ...
    try:
        ...
        log.error(f"Gemini API failed: {type(e).__name__}: {e}")
        return None


def build_system_prompt(narratives):
...
- Never make price predictions — signal strength only
- Always mention narrative names, confidence scores, and tokens when relevant
- Keep responses under 250 words"""


def ask_dual_ai(question, narratives):
    """Run Groq + Gemini in parallel, synthesize into one answer."""
    system = build_system_prompt(narratives)

    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        groq_future   = executor.submit(ask_groq,   question, narratives)
        gemini_future = executor.submit(ask_gemini, question, narratives, system)
    ...
    if groq_resp and gemini_resp and GEMINI_KEY:
        ...
        try:
            merge_prompt = f"""Two AI analysts answered: "{question}"

Analyst 1 (Groq/Speed): {groq_resp}

Analyst 2 (Gemini/Depth): {gemini_resp}

Synthesize into one definitive response under 200 words. No preamble."""

            merged = ask_gemini(merge_prompt, narratives, system=None)
            if merged:
                return merged
        except Exception:
            pass
    ...
    return groq_resp or gemini_resp or None
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Try Claude first
    reply = ask_dual_ai(text, narratives)
